...
class UserAccountServiceTest {
    ...
    void givenExistentUserId_whenSearching_thenReturnsOptionalUserData() {
        ...
        given(userAccountRepository.findByUserId(username)).willReturn(Optional.of(createUserAccount(username)));

        Optional<UserAccountDto> result = sut.searchUser(username);

        assertThat(result).isPresent();
        then(userAccountRepository).should().findByUserId(username);
    }
    ...
    void givenNonexistentUserId_whenSearching_thenReturnsOptionalUserData() {
        ...
        given(userAccountRepository.findByUserId(username)).willReturn(Optional.empty());
    ...
    void givenUserParams_whenSaving_thenSavesUserAccount() {
        ...
        UserAccount userAccount = createUserAccount("song");
        UserAccount savedUserAccount = createSigningUpUserAccount("song");
